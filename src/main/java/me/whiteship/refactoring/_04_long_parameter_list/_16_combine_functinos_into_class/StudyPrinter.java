...
public record StudyPrinter(int totalNumberOfEvents, List<Participant> participants) {
    ...
    public void print() throws IOException {
        try (FileWriter fileWriter = new FileWriter("participants.md");
            ...
            participants.forEach(p -> {
                String markdownForHomework = this.getMarkdownForParticipant(p.username(), p.homework());
    ...
    private String header() {
        ...
        for (int index = 1; index <= this.totalNumberOfEvents; index++) header.append(String.format(" %d주차 |", index));
        header.append(" 참석율 |\n");
    ...
    private String checkMark(Map<Integer, Boolean> homework) {
        ...
        for (int i = 1; i <= this.totalNumberOfEvents; i++) {
            if (homework.containsKey(i) && Boolean.TRUE.equals(homework.get(i))) line.append("|:white_check_mark:");
            else line.append("|:x:");
        }
